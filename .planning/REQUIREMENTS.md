...
- [ ] **SCRPT-02**: Context assembly — for selected lesson: pull lesson outline + top-5 relevant bootcamp transcript segments via keyword matching
- [ ] **SCRPT-03**: Claude API call (Anthropic SDK) — generates English narration script with AV two-column format and [SCREEN RECORDING], [IMAGE: desc], [VIDEO: desc], [VO], [PAUSE] markers
- [ ] **SCRPT-04**: Script review loop — display generated script in terminal, allow (a)ccept / (e)dit / (r)egenerate / (s)kip
- [ ] **SCRPT-05**: Save approved script to `scripts/M{n}L{n}_{slug}.md`
- [ ] **SCRPT-06**: Prompt caching — course content + bootcamp index cached across all 30 lesson generations (~$0.54 total cost)
...
| SCRPT-05 | Phase 3 | Pending |
| SCRPT-06 | Phase 3 | Pending |
